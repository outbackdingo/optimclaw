...
mod tests {
    ...
    async fn test_env_scrubbing_hides_secrets() {
        ...
        let secret_var = "OPTIMCLAW_TEST_SECRET_KEY";
    ...
    async fn test_env_scrubbing_custom_var_hidden() {
        ...
        unsafe { std::env::set_var("OPTIMCLAW_QA_TEST_SECRET", "supersecret123") };
        ...
        let output = result.result.get("output").unwrap().as_str().unwrap();
        assert!(
            !output.contains("OPTIMCLAW_QA_TEST_SECRET"),
        ...
        // Clean up
        unsafe { std::env::remove_var("OPTIMCLAW_QA_TEST_SECRET") };
    }
