...
5. Install with `optimclaw tool install path/to/tool.wasm`
...
When running `optimclaw tool auth <tool>`:
...
Both are first-class in the extension system (`optimclaw tool install` handles both), but they have different strengths.

**WASM Tools (OptimClaw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, OptimClaw can't prevent leaks
