...
use optimclaw::db::DatabaseHandles;
use optimclaw::secrets::{CreateSecretParams, SecretsCrypto, SecretsStore};
...
#[cfg(feature = "libsql")]
fn libsql_config(path: &std::path::Path) -> optimclaw::config::DatabaseConfig {
    optimclaw::config::DatabaseConfig {
        backend: optimclaw::config::DatabaseBackend::LibSql,
        url: secrecy::SecretString::from(String::new()),
        pool_size: 1,
        ssl_mode: optimclaw::config::SslMode::Prefer,
...
fn test_crypto() -> Arc<SecretsCrypto> {
    let key = secrecy::SecretString::from(optimclaw::secrets::keychain::generate_master_key_hex());
...
async fn connect_with_handles_returns_db_and_libsql_handle() {
    ...
    let (db, handles) = optimclaw::db::connect_with_handles(&config)
        .await
...
async fn connect_from_config_produces_working_db() {
    ...
    let db = optimclaw::db::connect_from_config(&config)
        .await
...
async fn secrets_store_from_handles_round_trips() {
    ...
    let (_db, handles) = optimclaw::db::connect_with_handles(&config)
        .await
        .expect("connect");

    let crypto = test_crypto();
    let store = optimclaw::secrets::create_secrets_store(crypto, &handles)
...
async fn db_create_secrets_store_standalone_round_trips() {
    ...
    let store = optimclaw::db::create_secrets_store(&config, crypto)
        .await
...
async fn both_secrets_factories_produce_compatible_stores() {
    ...
    let (_db, handles) = optimclaw::db::connect_with_handles(&config)
        .await
        .expect("connect");
    let store_a = optimclaw::secrets::create_secrets_store(Arc::clone(&crypto), &handles)
        .expect("store from handles");

    // Factory 2: db::create_secrets_store (standalone)
    let store_b = optimclaw::db::create_secrets_store(&config, crypto)
        .await
...
async fn extension_manager_with_process_manager_constructs() {
    ...
    use optimclaw::secrets::InMemorySecretsStore;
    use optimclaw::tools::ToolRegistry;
    use optimclaw::tools::mcp::McpProcessManager;
    use optimclaw::tools::mcp::McpSessionManager;
