...
mod advanced {
    ...
    use optimclaw::agent::routine::Trigger;
    ...
    async fn wait_for_routine_run(
        ...
        timeout: Duration,
    ) -> Vec<optimclaw::agent::routine::RoutineRun> {
    ...
    async fn user_steering() {
        let _cleanup = CleanupGuard::new().file("/tmp/optimclaw_steer_test.txt");
        let _ = std::fs::remove_file("/tmp/optimclaw_steer_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/optimclaw_steer_test.txt")
    ...
    async fn tool_error_recovery() {
        let _cleanup = CleanupGuard::new().file("/tmp/optimclaw_recovery_test.txt");
        let _ = std::fs::remove_file("/tmp/optimclaw_recovery_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/optimclaw_recovery_test.txt")
    ...
    async fn long_tool_chain() {
        let test_dir = "/tmp/optimclaw_chain_test";
        ...
        rig.send_message(
            "Create a daily log at /tmp/optimclaw_chain_test/log.md, \
    ...
    async fn routine_news_digest() {
        use optimclaw::llm::recording::{HttpExchange, HttpExchangeRequest, HttpExchangeResponse};
    ...
    async fn mcp_extension_lifecycle() {
        ...
        use optimclaw::extensions::{AuthHint, ExtensionKind, ExtensionSource, RegistryEntry};
        ...
        ext_mgr
            ...
            .create(
                TEST_USER_ID,
                optimclaw::secrets::CreateSecretParams::new(secret_name, "mock-access-token")
    ...
    async fn bootstrap_onboarding_clears_bootstrap() {
        ...
        let deser_result =
            serde_json::from_str::<optimclaw::profile::PsychographicProfile>(&stored.content);
        assert!(
            deser_result.is_ok(),
